...
export const useHeroOperations = (connectedAddress, fetchHeroes, setHeroes) => {
  ...
  const updatePrice = useCallback(
    async (heroId, newPrice) => {
      ...
      try {
        ...
        try {
          const lastUpdateTime = await HONKMarketplaceContract.methods
            .lastPriceUpdateTime(heroId)
            .call();
          ...
          if (currentTime < nextAllowedUpdate) {
            ...
            throw new Error(
              `Price update cooldown active. You can update the price in ${timeMessage}. Please wait and try again.`
            );
          }
        } catch (cooldownError) {
        ...
        let gasEstimate;
        try {
          gasEstimate = await HONKMarketplaceContract.methods
            .updatePrice(heroId, priceInWei)
            .estimateGas({
          ...
          if (errorMessage.includes('Price update too soon')) {
            throw new Error(
              "Price update cooldown active. You can only update a hero's price once every 15 minutes. Please wait and try again."
            );
          }
        ...
        const tx = await HONKMarketplaceContract.methods.updatePrice(heroId, priceInWei).send({
          from: connectedAddress,
          gasLimit: gasLimit,
        });
        ...
        if (errorMessage.includes('revert')) {
          errorMessage =
            'Transaction was reverted by the contract. Please check if the hero is still listed and you own it.';
  ...
  const checkAndCancelIfMoved = useCallback(
    async (heroId) => {
      if (!connectedAddress || !heroId) {
        return {
          success: false,
      ...
      try {
        ...
        if (heroData.network && heroData.network !== 'dfk') {
          ...
          if (result.success) {
            toast.info(
              `Hero #${heroId} was automatically unlisted because it moved to ${realmName}`
            );
          }
